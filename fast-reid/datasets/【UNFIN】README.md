...
Fastreid对于一些数据集原生支持.数据集假定存在于环境变量`FASTREID_DATASETS`指定的目录下.fastreid预期在下面描述的目录结构下找到数据集.
...
您可以使用`export FASTREID_DATASETS=/path/to/datasets/`来设定内置数据集的位置.如果未设置，默认位于当前工作目录下的相对目录`datasets/`.
