# FastReID Model Zoo and Baselines | FastReID 动物园模型与基准线
...
该文档收集了用fastreid训练的基准线。所有数据皆来源于一张NVIDIA V100 GPU的训练结果
使用的软件是PyTorch 1.6，CUDA 10.1.

除这些官方基准模型外，你可以在[projects/](https://github.com/JDAI-CV/fast-reid/tree/master/projects).找到更多模型。

### How to Read the Tables | 如何阅读这些表
...
- "Name"列连接着该配置文件的目录.
以这个配置文件与单卡运行`tools/train_net.py`会再现这个模型.

### Common Settings for all Person reid models | 适用于所有面部重识别模型的设定
...
[一些用于深度面部重识别的技巧与强大的基线](http://openaccess.thecvf.com/content_CVPRW_2019/papers/TRMTMCT/Luo_Bag_of_Tricks_and_a_Strong_Baseline_for_Deep_Person_CVPRW_2019_paper.pdf). CVPRW2019, Oral.

**AGW**:

[ReID-Survey with a Powerful AGW Baseline](https://github.com/mangye16/ReID-Survey).

[用强大的AGW基线进行的重识别研究](https://github.com/mangye16/ReID-Survey).
...
[利用多粒度面部重识别学习Discriminative Features](https://arxiv.org/abs/1804.01438v1)
...
1. Circle loss | 园损失
2. Freeze backbone training | 冻结骨干网训练
3. Cutout data augmentation & Auto Augmentation | 剪切数据增加与自动增加
4. Cosine annealing learning rate decay | 余弦重组学习率的衰减
5. Soft margin triplet loss | 软盈余三联损失

Bag of Specials(BoS):

1. Non-local block | 非本地（代码）块
2. GeM pooling | GeM池
### Market1501 Baselines | Market1501 基准线
